...
class NodeStencilFour(Node):
    ...
    def getString(self, i, j, k, pad, plane, bias=0):
        i0 = i1 = i2 = i3 = i
        j0 = j1 = j2 = j3 = j
        k0 = k1 = k2 = k3 = k
        if (self.dim == 0):
            i0 += -1-self.loc[0] + bias
            i1 +=   -self.loc[0] + bias
            i2 += +1-self.loc[0] + bias
            i3 += +2-self.loc[0] + bias
        elif (self.dim == 1):
            j0 += -1-self.loc[1] + bias
            j1 +=   -self.loc[1] + bias
            j2 += +1-self.loc[1] + bias
            j3 += +2-self.loc[1] + bias
        elif (self.dim == 2):
