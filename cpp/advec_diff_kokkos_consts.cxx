...
namespace
{
    ...
    struct diff
    {
        ...
        {
            constexpr Real cg0 = Real(  1./24.);
            constexpr Real cg1 = Real(-27./24.);
            constexpr Real cg2 = Real( 27./24.);
            constexpr Real cg3 = Real( -1./24.);

            ut(k, j, i) += visc * (
                    + cg0*( cg0*u(k-3, j, i) + cg1*u(k-2, j, i) + cg2*u(k-1, j, i) + cg3*u(k  , j, i) )
                    + cg1*( cg0*u(k-2, j, i) + cg1*u(k-1, j, i) + cg2*u(k  , j, i) + cg3*u(k+1, j, i) )
                    + cg2*( cg0*u(k-1, j, i) + cg1*u(k  , j, i) + cg2*u(k+1, j, i) + cg3*u(k+2, j, i) )
                    + cg3*( cg0*u(k  , j, i) + cg1*u(k+1, j, i) + cg2*u(k+2, j, i) + cg3*u(k+3, j, i) )

                    + cg0*( cg0*u(k, j-3, i) + cg1*u(k, j-2, i) + cg2*u(k, j-1, i) + cg3*u(k, j  , i) )
                    + cg1*( cg0*u(k, j-2, i) + cg1*u(k, j-1, i) + cg2*u(k, j  , i) + cg3*u(k, j+1, i) )
                    + cg2*( cg0*u(k, j-1, i) + cg1*u(k, j  , i) + cg2*u(k, j+1, i) + cg3*u(k, j+2, i) )
                    + cg3*( cg0*u(k, j  , i) + cg1*u(k, j+1, i) + cg2*u(k, j+2, i) + cg3*u(k, j+3, i) )

                    + cg0*( cg0*u(k, j, i-3) + cg1*u(k, j, i-2) + cg2*u(k, j, i-1) + cg3*u(k, j, i  ) )
                    + cg1*( cg0*u(k, j, i-2) + cg1*u(k, j, i-1) + cg2*u(k, j, i  ) + cg3*u(k, j, i+1) )
                    + cg2*( cg0*u(k, j, i-1) + cg1*u(k, j, i  ) + cg2*u(k, j, i+1) + cg3*u(k, j, i+2) )
                    + cg3*( cg0*u(k, j, i  ) + cg1*u(k, j, i+1) + cg2*u(k, j, i+2) + cg3*u(k, j, i+3) ) );
        }
    };
}
...
int main(int argc, char* argv[])
{
    Kokkos::initialize(argc, argv);
    {
        ...
        Range_3d_cpu range_3d_cpu({4, 4, 4}, {ktot-4, jtot-4, itot-4}, {4, 4, 0});
